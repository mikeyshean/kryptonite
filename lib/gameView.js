(function () {
  ...
  var GameView = window.Flappy.GameView = function (options) {
    ...
    this.$el = options.$el
    // this.$getLeaders = this.$el.find(".view-leaders")
    ...
    var intervalId = setInterval(function () {
      ...
      if (game.gameOver) {
        ...
        setTimeout(function () {
          that.showScore();
          that.$restart.show();
          that.$submitScore.show();

          that.$submitScore.one("click", function () {
            var name = prompt("Name: ");
            that.submitScore(name);
            that.$restart.hide();
            that.$submitScore.hide();
          })

          that.$restart.one("click", function (e) {
            that.$restart.hide();
            that.$submitScore.hide();
            that.run()
          })
